...
class TestGoToPersonalAccountPage:
    def test_sign_in_by_enter_button(self, driver):
        ...
        expected_text = WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.XPATH, ".//p[text()='В этом разделе вы можете изменить свои персональные данные']"))).text # Находим кнопку
